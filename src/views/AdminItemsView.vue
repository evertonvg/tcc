  ...
  <div class="flex flex-col">
    ...
    <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
      <div class="overflow-hidden">
        <!-- animes  -->
        <table class="min-w-full" v-if="$route.params.item=='animes'">
          ...
          <tbody>
            <tr class="border-b" v-for="(anime,index) in items" :key="index">
        ...
        <!-- categories  -->
        <table class="min-w-full" v-if="$route.params.item=='categories'">
          <thead class="border-b">
            <tr>
            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4 text-center">
                id
...
<div id="defaultModal" tabindex="-1" class="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full justify-center items-center flex bg-black" aria-modal="true" role="dialog" v-if="addModal || updateModal">
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto" >
        <!-- Modal content -->
        <!-- modal animes  -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 opacity-100" v-if="$route.params.item=='animes'">
            <!-- Modal header -->
            ...
            <div class="p-6 space-y-6">
                <div class="relative mb-4">
                    <label for="name" class="leading-7 text-sm text-gray-600 block text-left">Name <span class="text-red-900 font-bold" v-show="v$.category.name.$error"> (Nome requerido)</span></label>
                    <input type="text" id="name" name="name" v-model="category.name" @change="v$.category.name.$touch()" :class="['w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out',v$.anime.name.$error ? 'border-red-900' : '']">
                </div>
                <div class="relative mb-4">
                    <label for="description" class="leading-7 text-sm text-gray-600 block text-left">Description<span class="text-red-900 font-bold" v-show="v$.category.description.$error"> (Descrição requerida)</span></label>
                    <textarea id="description" name="description" v-model="category.description" @change="v$.category.description.$touch()" :class="[`w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out`,v$.anime.description.$error ? 'border-red-900' : '']"></textarea>
                </div>
                <div class="relative mb-4">
                    <label for="image" class="leading-7 text-sm text-gray-600 block text-left">Image</label>
                    <input type="file" id="image" name="image" ref="fileImageCategory" :class="[`w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out`]"> <br>
        ...
        <!-- modal categorias  -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 opacity-100" v-if="$route.params.item=='categories'">
            <!-- Modal header -->
            <div class="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="defaultModal" @click="closeModals">
                    <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
