<template>
    ...
    <transition name="fade">
        <div class="container mx-auto  grid grid-cols-2 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-10 gap-4 px-5 xl:px-0 clear-both mt-16" v-show="enableCats">
            <div v-for="(cat,index) in categories" :key="index" class="mb-[0.125rem] block min-h-[1.5rem] pl-[1.5rem]">
                <input
                    ...
                    :id="cat.name+'cat'" />
                <label
                    class="inline-block pl-[0.15rem] hover:cursor-pointer float-left font-bold text-left"
                    :for="cat.name+'cat'">
                    {{ cat.name }}
                </label>
            </div>
